/*
 Product of array returns 1,-1, or 0
 if 1 if sum is positive, -1 is negative, 0 if 0
 */

 const arraySign = (nums) => {
     let sum;
     if(nums.length){
         sum = Math.sign(nums[0]);
     }

     for (let i = 1; i < nums.length; i++){
        sum = Math.sign(sum * nums[i])
     }
